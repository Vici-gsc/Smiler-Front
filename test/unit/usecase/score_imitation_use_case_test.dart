...
import 'package:smiler/domain/usecase/score_imitation_use_case.dart';
import 'package:test/test.dart';

@GenerateNiceMocks([
  MockSpec<ImitationRepositoryImpl>(),
  MockSpec<HistoryRepositoryImpl>(),
])
import 'score_imitation_use_case_test.mocks.dart';
...
main() {
  group("getScoringResult", () {
    test("요청 성공시 채점 결과를 가져온다. - 정답인 경우", () async {
      ...
      final useCase = ScoreImitationUseCase(repository, historyRepository);

      when(repository.isCorrectImitation(any, any)).thenAnswer(
    ...
    test("요청 실패시 에러를 반환한다.", () async {
      ...
      when(repository.isCorrectImitation(any, any))
